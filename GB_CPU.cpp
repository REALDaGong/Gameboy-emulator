...
void Z80::InitOpCodeList() {
	...
	OpCode[0xE8] = [&]()->int {
		...
		SetFlag(FLAG_ZERO, 0);
		SetFlag(FLAG_NEGA, 0);
		SetFlag(FLAG_CARY, (((_REG.SP&0xFF)+by) & 0x100)!=0);
		SetFlag(FLAG_HACA, ((_REG.SP&0xf+by&0xf)&0x10)!=0);
	...
	//p95
	//flagf
	//BUG OVER HERE BUG OVER HERE BUG OVER HERE BUG OVER HERE BUG OVER HERE BUG OVER HERE  BUG OVER HERE BUG OVER HERE  BUG OVER HERE BUG OVER HERE
	OpCode[0x27] = [&]()->int {
		GB_DB tmp = _REG.A;
